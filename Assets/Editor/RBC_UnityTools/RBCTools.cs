...
class RBCTools : EditorWindow {
    ...
    void OnGUI () {
        GUILayout.Space (20); c_select = EditorGUILayout.Popup ("Select Function", c_select, c_options);
        GUILayout.Space (10); curSearchOption = EditorGUILayout.Popup ("Search Options", curSearchOption, searchOption);
        GUILayout.Space (10);

        if (c_select == 0) {
            GUILayout.Label ("UPDATE GAMEOBJECT(S) NAME", EditorStyles.boldLabel);
            GUILayout.Space (10); ObjecToSearch = EditorGUILayout.TextField ("Search by name", ObjecToSearch);
            GUILayout.Space (5); NewName = EditorGUILayout.TextField ("Set new name", NewName);
            GUILayout.Space (5); ExcludeParents = EditorGUILayout.Toggle ("Exclude parents", ExcludeParents);
            GUILayout.Space (5); CustomSuffix = EditorGUILayout.TextField ("Custom Suffix", CustomSuffix);
            GUILayout.Space (10); GUI.backgroundColor = Color.grey;
        }
            ...
            GUILayout.Space (10); ObjecToSearch = EditorGUILayout.TextField ("Search by name", ObjecToSearch);
            GUILayout.Space (5); ExcludeParents = EditorGUILayout.Toggle ("Exclude parents", ExcludeParents);
            GUILayout.Space (5); NewTag = EditorGUILayout.TagField ("Set new tag", NewTag);
            GUILayout.Space (10); GUI.backgroundColor = Color.grey;
        }
            ...
            GUILayout.Space (10); ObjecToSearch = EditorGUILayout.TextField ("Search by name", ObjecToSearch);
            GUILayout.Space (5); ExcludeParents = EditorGUILayout.Toggle ("Exclude parents", ExcludeParents);
            GUILayout.Space (5); SetStatic = EditorGUILayout.Toggle ("Set object as static", SetStatic);
            GUILayout.Space (10); GUI.backgroundColor = Color.grey;
        }
            ...
            GUILayout.Space (10); ObjecToSearch = EditorGUILayout.TextField ("Search by name", ObjecToSearch);
            GUILayout.Space (5); ExcludeParents = EditorGUILayout.Toggle ("Exclude parents", ExcludeParents);
            GUILayout.Space (5); curComponent = EditorGUILayout.Popup ("Select Component", curComponent, component_list);
            GUILayout.Space (10); GUI.backgroundColor = Color.grey;
        }
